...
function AddUsers() {
  ...
  const showModal = async (user) => {
    setCurrentUser(user);
    try {
      const response = await axios.get(`http://localhost:5000/api/users/getUser/${user._id}`);
  ...
  const handleUpdate = async (values) => {
    try {
      ...
      await axios.put(`http://localhost:5000/api/users/updateUser/${currentUser._id}`, values);
